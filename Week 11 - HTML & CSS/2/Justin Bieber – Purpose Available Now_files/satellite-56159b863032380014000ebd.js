...
_sdi.yt = {
  ...
  sc: {
    track: true,
    s: function(){
      return window.s_gi(window._rsid)
    }
  },
  timeout: 1000,
  /****** END Configuration ******/
  ...
  init: function() {
    ...
    if (api === false) {
      ...
      _satellite.loadScript('https://www.youtube.com/iframe_api',function(){
        _sdi.yt.ready('api');
      });
    }
    else {
  ...
  doubleCheck: function(){
    ...
    for(var v in vids){
      var vid = vids[v];
      _satellite.cssQuery('iframe[src*="'+v+'"]', function(ifr){
        if(ifr.length == 0){
          if(sc.track == true){
            if(vid.state == 'playing' || vid.state == 'paused'){
              _satellite.notify('iframe for video "'+vid.info.videoData.title+'" not found, ending video', 1);
              sc.s().Media.stop(vid.info.videoData.title, vid.info.currentTime);
              sc.s().Media.close(vid.info.videoData.title);
              vid.state = 'completed';
              try {
                delete vids[v];
              }
              catch(err){
                _satellite.notify('unable to delete video', 1);
              }
            }
          }
        }
    ...
    setTimeout(function(){_sdi.yt.doubleCheck()},_sdi.yt.timeout);
  }
};
_sdi.yt.init();
...
_satellite.domReady(function(){
  jQuery('a[href*="youtube.com/embed"]').each(function(idx,yt){
    var href = jQuery(yt).attr('href');
    if(href.indexOf('enablejsapi') == -1){
      jQuery(yt).attr('href', href.replace(/http:/,'https:')+(href.indexOf('?')>-1?'&':'?')+'enablejsapi=1');
    }
  });
  _sdi.yt.ready('dom');
});
